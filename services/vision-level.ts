...
export async function detectLevelFromImage(
    ...
    if (Platform.OS !== 'android' || !PixelSamplerModule) {
    ...
    try {
        ...
        let arcBounds = await scanForArcBounds(uri, startY, endY, width, height);
...
function findWhiteDotInRow_unused(pixels: number[][]) { return null; } // Legacy removal stub
